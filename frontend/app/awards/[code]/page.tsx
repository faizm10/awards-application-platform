...
const AwardDetailPage = () => {
  ...
  useEffect(() => {
    const fetchAwardAndFields = async () => {
      ...
      // Fetch award details
      const { data: awardData, error: awardError } = await supabase.from("awards").select("*").eq("code", code).single()
            ...
            <section className="card-modern p-6">
              ...
              <div className="space-y-4">
                <p className="text-sm text-muted-foreground mb-4">
                  The following information will be required when you apply for this award:
                </p>
                <div className="grid gap-3">
                  {requiredFields.map((field) => (
                    <div key={field.id} className="flex items-start gap-3 p-3 bg-muted/30 rounded-lg">
                      <div className="flex-shrink-0 mt-1">
                        ...
                        ) : field.type === "textarea" || field.field_config?.type === "essay" ? (
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded capitalize">
                            {field.field_config?.type === "essay" ? "Essay" : field.type}
                          </span>
                        </div>
                        {field.field_config?.question && (
                          <p className="text-xs text-muted-foreground mt-1 italic">"{field.field_config.question}"</p>
                        )}
